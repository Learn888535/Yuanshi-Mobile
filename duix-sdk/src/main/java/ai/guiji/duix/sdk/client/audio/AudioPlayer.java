...
public class AudioPlayer {
    ...
    private float mVolume = 1.0f;  // 存储当前音量，在startPlay()后恢复
    private boolean hasStarted = false;  // AudioTrack是否已经启动过（防止未启动时stop/flush导致状态异常）
    ...
    public void startPlay(){
        ...
        try {
            if (hasStarted) {
                // 已经启动过：stop/flush/play 标准循环
                audioTrack.flush();
            }
            audioTrack.play();
            hasStarted = true;
            // 某些Android版本在stop()/play()后会重置音量，这里确保恢复
