...
public class RenderThread extends Thread {
    ...
    private boolean mPushDone = false;              // 标记所有音频数据已推送完毕
    ...
    private long renderStep() {
        ...
        if (frame != null) {
            boolean hasValidSession = mCurrentBnfSession > 0;
            int readyCnt = hasValidSession ? scrfdncnn.readycnt(mCurrentBnfSession) : 0;
            if (readyCnt > 0 && audioPlayer != null && hasValidSession){
                ...
                // Fallback: If all audio has been pushed (mPushDone) but NCNN hasn't produced ready frames,
                // still start audio playback so the user can hear the sound (without lip sync).
                if (mPushDone && hasValidSession && audioPlayer != null && mLastBnfSession != mCurrentBnfSession) {
                    mLastBnfSession = mCurrentBnfSession;
                    Logger.w("renderStep fallback: starting audio playback without NCNN ready frames (readyCnt=" + readyCnt + ")");
                    audioPlayer.startPlay();
                }
                isLip = false;
                Logger.i("renderStep no audio session=" + mCurrentBnfSession + " readyCnt=" + readyCnt + " hasValid=" + hasValidSession);
    ...
    private void handleStartPushAudio(){
        Logger.i("handleStartPushAudio oldSession=" + mCurrentBnfSession);
        if (mCurrentBnfSession > 0){
            scrfdncnn.finsession(mCurrentBnfSession);
        }
        mCurrentBnfSession = scrfdncnn.newsession();
        mPushDone = false;
        Logger.i("handleStartPushAudio newSession=" + mCurrentBnfSession);
        if (audioPlayer != null && isRendering){
            audioPlayer.pushStart();
            Logger.i("handleStartPushAudio pushStart OK");
        }
    }
    ...
    private void handlePushAudio(byte[] data){
        if (audioPlayer != null && isRendering){
            ...
            Logger.w("handlePushAudio SKIP: audioPlayer=" + (audioPlayer!=null) + " isRendering=" + isRendering);
        }
    }

    private void handleStopPushAudio() {
        Logger.i("handleStopPushAudio session=" + mCurrentBnfSession + " isRendering=" + isRendering);
        if (scrfdncnn != null && isRendering && mCurrentBnfSession > 0){
            scrfdncnn.finsession(mCurrentBnfSession);
        }
        if (audioPlayer != null){
            audioPlayer.pushDone();
            Logger.i("handleStopPushAudio pushDone OK");
        }
        mPushDone = true;
    }
